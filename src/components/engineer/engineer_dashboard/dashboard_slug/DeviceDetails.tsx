// filepath: d:\new-clients\abdullah-shahid\smart_industry_meter\meter-frontend\src\components\engineer\engineer_dashboard\engineer_slug\DeviceDetails.tsx
...
import { DeviceData } from "@/store/slices/engineer/engineerTypes";
import { deleteDevice, generateMeterReport, generateAlarmMeterReport } from "@/store/slices/admin/adminThunks";
import { toast } from "sonner";

// Reusable Header Stats component to show the meter UI
function HeaderStats({ deviceData }: { deviceData: any }) {
...
export default function DeviceDetails() {
  console.log("DeviceDetails component rendered");
  const { slug } = useParams();
  const dispatch = useDispatch<AppDispatch>();
  const navigate = useNavigate();
  const isLoading = useSelector((state: RootState) => state.engineer.isLoading);
...
function GeneratorTab({ deviceData }: any) {
  return (
    <div className="flex flex-col gap-6 py-4">
      <div className="flex justify-between items-center">
        <div className="flex items-center">
